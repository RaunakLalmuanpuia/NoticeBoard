<template>
  <v-card flat style="background-color: #f7f9f7">
    ...
    <v-card-title>Log Hut (Day & Night)</v-card-title>
    ...
    <v-card-text>
      ...
      <div
        v-if="
          ...
          checkingResult != '' && checkingResult != 'Check-in and Check-out dates cannot be the same.'
        "
      >
        <v-row>
          <v-col md="6">
    ...
    showCheckoutDate(date) {
     ...
     checkDate() {
      ...
      if(self.date_from === self.date_to){
        self.checkingResult = 'Check-in and Check-out dates cannot be the same.';
        return;
      }
      else if (self.date_from != "" && self.date_to != "" ) {
        self.$axios
          .get(
          ...
          .then(function (res) {
            ...
            console.log(res.data.hutsAvailable);
            console.log(res.data.no_of_days);
